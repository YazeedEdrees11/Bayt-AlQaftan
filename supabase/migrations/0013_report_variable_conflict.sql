-- =============================================================================
-- 0013 — Resolve plpgsql name shadowing in the guarded report functions
--
-- 0012 converted the reporting functions from `language sql` to plpgsql so they
-- could refuse an unauthorized caller. That conversion has a consequence I did
-- not account for: in plpgsql the OUT columns declared by `returns table (...)`
-- become variables, so a body that also selects a column of the same name — for
-- example `stock_cost` in get_management_kpis — no longer resolves. Postgres
-- reports it honestly rather than guessing:
--
--     column reference "stock_cost" is ambiguous
--     It could refer to either a PL/pgSQL variable or a table column
--
-- `#variable_conflict use_column` settles it in favour of the column, which is
-- what every one of these bodies means. The parameters are all `p_`-prefixed
-- and so cannot collide either way.
--
-- Same bodies, same guard, same figures — only the resolution rule is stated.
-- =============================================================================
