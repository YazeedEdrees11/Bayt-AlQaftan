...
import { searchExchangeVariantsAction } from "@/app/actions/returns";
import { formatMoney, formatNumber } from "@/lib/utils/format";
import { cn } from "@/lib/utils/cn";
import type { SellableVariant } from "@/types/sales";

/**
 * Product lookup for the replacement leg of an exchange.
 *
...
export function ExchangeVariantPicker({
  ...
  /** Variants already on the exchange, so they are not offered twice. */
  chosen: string[];
}) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      startTransition(async () => {
        const result = await searchExchangeVariantsAction(search);
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>اختيار المنتج البديل</DialogTitle>
          <DialogDescription>
        ...
        <div className=" max-h-[26rem] space-y-2 overflow-y-auto">
          {isPending && !loaded ? (
            ...
            results.map((variant) => {
              ...
              const remaining = variant.current_stock;
              const unavailable = remaining <= 0 || already;
